...
% end%normalization
% Z_all_norm_d=zeros(11,size(Z_all_norm,2));
% for rr=1:size(Z_all_norm,1)
%     temp=[0:1:10];
%      Z_all_norm_d(all(rr,2)+1,:)=Z_all_norm_d(all(rr,2)+1,:)+Z_all_norm(rr,:);
% end
% 
% [x,m]=max(Z_all_norm_d);
% p=[p;x];
% for ww=1:size(x,2)
